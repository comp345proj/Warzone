...
void testLoggingObserver() {
    ...
    std::cout << "CommandProcessor is ILoggable: " << (dynamic_cast<ILoggable*>(&cp) != nullptr) << std::endl;
    std::cout << "Command is ILoggable: " << (dynamic_cast<ILoggable*>(&cmd) != nullptr) << std::endl;
    std::cout << "Order (Deploy) is ILoggable: " << (dynamic_cast<ILoggable*>(&deployOrder) != nullptr) << std::endl;
    std::cout << "OrdersList is ILoggable: " << (dynamic_cast<ILoggable*>(&ordersList) != nullptr) << std::endl;
    std::cout << "GameEngine is ILoggable: " << (dynamic_cast<ILoggable*>(&ge) != nullptr) << std::endl;

    std::cout << "CommandProcessor is Subject: " << (dynamic_cast<Subject*>(&cp) != nullptr) << std::endl;
    std::cout << "Command is Subject: " << (dynamic_cast<Subject*>(&cmd) != nullptr) << std::endl;
    std::cout << "Order (Deploy) is Subject: " << (dynamic_cast<Subject*>(&deployOrder) != nullptr) << std::endl;
    std::cout << "OrdersList is Subject: " << (dynamic_cast<Subject*>(&ordersList) != nullptr) << std::endl;
    std::cout << "GameEngine is Subject: " << (dynamic_cast<Subject*>(&ge) != nullptr) << std::endl;
    ...
    // Issue order by playing the card; this will add an order to bob's orders list and notify
    ...
    // 6) When GameEngine changes state, ensure the new state is output to the log file
    ge.Attach(logger);
    ge.setState(StateType::PLAYERS_ADDED);
    // GameEngine doesn't automatically call Notify in this implementation, so call it explicitly
    ge.Notify(&ge);
