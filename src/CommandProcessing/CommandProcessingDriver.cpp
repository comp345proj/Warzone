...
void testCommandProcessor(int argc, char* argv[]) {
    ...
    if (argc <= 1) {
        std::cout << "No arguments provided. Usage: ./program [-console] OR [-file filename]" << std::endl;
        return;
    }
    ...
    if (commandOption == "-file") {
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    std::cout << "\nRead command: " << cmd.getCommand() << std::endl;
                    bool isValid = fcpa.validate(cmd, currentState);
                    
                    std::cout << "Valid in current state: " << (isValid ? "yes" : "no") << std::endl;
                    std::cout << "Effect: " << cmd.getEffect() << std::endl;
                    
                    // Update state based on command
                    if (isValid) {
                        if (cmd.getCommand().find("loadmap") == 0) currentState = StateType::MAP_LOADED;
                        else if (cmd.getCommand() == "validatemap") currentState = StateType::MAP_VALIDATED;
                        else if (cmd.getCommand().find("addplayer") == 0 && currentState == StateType::MAP_VALIDATED) currentState = StateType::PLAYERS_ADDED;
                        else if (cmd.getCommand() == "gamestart") currentState = StateType::ASSIGN_REINFORCEMENT;
                        else if (cmd.getCommand() == "quit") break;
                    }
        ...
        for (const auto& cmd : testCommands) {
            ...
            // Update state based on command
            if (isValid) {
                if (cmd.find("loadmap") == 0) currentState = StateType::MAP_LOADED;
                else if (cmd == "validatemap") currentState = StateType::MAP_VALIDATED;
                else if (cmd.find("addplayer") == 0) currentState = StateType::PLAYERS_ADDED;
                else if (cmd == "quit") break;
            }
        }
        
    } else {
        std::cout << "Invalid option. Use -console or -file <filename>" << std::endl;
        return;
    }
