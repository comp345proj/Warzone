...
void testOrderExecution() {
    ...
    std::cout << "Valid deploy order validation: " << (validDeploy->validate() ? "passed" : "failed") << std::endl;
    std::cout << "Invalid deploy order validation: " << (!invalidDeploy->validate() ? "passed" : "failed") << std::endl;
    
    validDeploy->execute();
    invalidDeploy->execute();
    
    std::cout << "\n2. Testing Advance Order and Territory Conquest" << std::endl;
    ...
    std::cout << "T3 owner: " << (t3->getPlayer() == p1 ? "Player 1 (Conquered)" : "Player 2") << std::endl;
    ...
    // The card should have been awarded in the previous advance execution if conquest was successful
    ...
    std::cout << "Initial owner of T2: " << (t2->getPlayer() == p1 ? "Player 1" : "Other") << std::endl;
    
    blockade->execute();
    
    std::cout << "After blockade:" << std::endl;
    std::cout << "Armies in T2: " << t2->getArmies() << std::endl;
    std::cout << "Owner of T2: " << (t2->getPlayer() == Blockade::getNeutralPlayer() ? "Neutral" : "Not Neutral") << std::endl;
...
void testOrdersList() {
	...
	Order* advance = new Advance(dummyPlayer, dummyTerritory1, dummyTerritory2, 3);
	Order* bomb = new Bomb(dummyPlayer, dummyTerritory2);
	Order* blockade = new Blockade(dummyPlayer, dummyTerritory1);
	Order* airlift = new Airlift(dummyPlayer, dummyTerritory1, dummyTerritory2, 2);
